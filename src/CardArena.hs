...
--                                Bool is true if the current round is still going or false if it has been won, and Int represents the number of consecutive rounds won by the player
...
play (State pWon cWon pCard cCard winner) (Mem wins oldGame) =
   do
      ...
      if (checkWinner pWon cWon == 1) then do
         ...
         if (y == "y" || y == "Y")
            then do
            play beginState (Mem (wins + 1) oldGame)
         else do
            return (State pWon cWon pCard cCard winner)
              ...
              if ((length chosenCards) > 0)
                 ...
                 if (winner) then do
                   ...
                   play (State (chosenCard:pWon) cWon newpCard newcCard winner) (Mem wins oldGame)
                 else do
                   putStrLn("You lost the round!")
                   putStrLn("Your card " ++ (cName chosenCard) ++ " LOST against the computer's card " ++ (cName computerCard))
                   play (State pWon (computerCard:cWon) newpCard newcCard winner) (Mem wins oldGame)
              else do
